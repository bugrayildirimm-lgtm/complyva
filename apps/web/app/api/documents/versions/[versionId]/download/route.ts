...
export async function GET(req: NextRequest, { params }: { params: Promise<{ versionId: string }> }) {
  ...
  const fileRes = await fetch(`${API_BASE}/documents/versions/${versionId}/download`, { headers: { "x-org-id": me.org_id, "x-user-id": me.id, "x-role": me.role } });
  ...
  return new NextResponse(buffer, { headers: { "Content-Type": contentType, "Content-Disposition": contentDisposition } });
}
