...
function HeatMapView({ risks }: { risks: Risk[] }) {
  ...
  for (const r of activeRisks) {
    ...
    if (view === "inherent") {
      ...
      if (!r.residual_score) continue;
      if (r.residual_likelihood && r.residual_impact) {
        l = r.residual_likelihood;
        i = r.residual_impact;
      } else {
        // Derive position from residual_score using control effectiveness ratio
        const ratio = r.inherent_score > 0 ? r.residual_score / r.inherent_score : 1;
        l = Math.max(1, Math.round(r.likelihood * ratio));
        i = Math.max(1, Math.round(r.impact * ratio));
      }
    }
    if (l && i) {
      const key = `${l}-${i}`;
  ...
  const residualCount = activeRisks.filter((r) => r.residual_score).length;
            ...
            {[
              ...
              const bandCount = activeRisks.filter((r) => {
                ...
                if (view === "inherent") {
                  ...
                  if (!r.residual_score) return false;
