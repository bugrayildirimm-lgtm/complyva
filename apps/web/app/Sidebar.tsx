...
export default function Sidebar() {
  ...
  useEffect(() => {
    async function fetchCount() {
      try {
        const { getPendingApprovalCount } = await import("../lib/api");
        const data = await getPendingApprovalCount();
    ...
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <a key={item.href} href={item.href} className={`sidebar-link ${isActive ? "active" : ""}`} style={{ position: "relative" }}>
              {item.icon}
              {item.label}
              {showBadge && (
                <span style={{
